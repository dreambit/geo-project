...
.directive('map', function ($timeout, Svg) {
  return {
    ...
    link: function ($scope) {
      var createUrl = function(location, quality) {
        return 'maps/' + (location || $scope.location) + (quality ? 'High' : 'Low') + '.svg';
      },
      ...
      */
      onSVGReady = function (event) {
        ...
        initPopulation();
        populationVisibility($scope.showPopulation.value);
        applyCSS();
      ...
      */
      ...
      $scope.$watch('quality.value', function(quality) {
        $scope.url = createUrl(null, quality);
      });

      $scope.$watch('showPopulation.value', function(showPopulation) {
